...
heros.forEach(hero => {
    ...
    hero.lasteName = name[name.length - 1];
    delete hero.name;
    console.log(hero);
})
...
// Use.map() on the heros2 array to
// return a new array
// Inside this method remove the key 'name'
// and add two new keys 'firstName'
// and 'lastName'
const heros2 = [{
        name: ['Bruce', 'Wayne'],
        hero: 'Batman'
...
const newHeroes = heros2.map(hero => {
    const name = hero.name;
    hero.firstName = name[0];
...
// Using.reduce() on the cartoonIds array
// return a new object
...
// Using reduce to understand  accumulator
const result4 = cartoonIds.reduce((accumulator, item) => {
        if (item.id) {
